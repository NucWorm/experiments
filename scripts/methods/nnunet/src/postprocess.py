...
def process_heatmap_file(heatmap_path, output_folder, gt_folder,min_distance=5, threshold_abs=0.1):
    ...
    # Generate the output file name, e.g., "xxx.tiff" -> "xxx_points.npy"
    base_name = os.path.splitext(os.path.basename(heatmap_path))[0]
    case_group = base_name.split("_")[0]  # Keep this for gt_folder logic
    # Keep the full base_name (including case_group) for congruity with input files
    save_folder = output_folder  # No extra subdirectory nesting
    # Only modify gt_folder if it's not None
